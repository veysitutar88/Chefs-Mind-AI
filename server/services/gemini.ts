...
try {
  ...
  console.error(
    'Failed to import GoogleGenerativeAI:',
...
export async function analyzeWithGemini(
  ...
  let model =
    modelName ||
    (agentType === 'accountant' ? 'gemini-2.5-pro' : 'gemini-2.5-flash');
...
export async function generateWithGemini(
  ...
  try {
    if (type === 'image') {
      ...
      console.log(
        '🎬 Video generation with Veo 3 is not yet available in this region'
      );

      // Veo 3 is still in limited preview
      const placeholderData =
        'data:text/plain;base64,' +
        Buffer.from(`Veo 3 generation requested for: ${prompt}`).toString(
          'base64'
        );
    ...
    if (
      ...
      console.warn(
        '⚠️ Vertex AI API not enabled or permission denied for this project'
      );
      console.warn(
        '💡 To enable Vertex AI: Visit Google Cloud Console -> APIs & Services -> Enable Vertex AI API'
      );
    ...
    const placeholderData =
      'data:text/plain;base64,' +
      Buffer.from(
        `${type === 'image' ? 'Imagen 3' : 'Veo 3'} generation failed: ${errorMessage}`
      ).toString('base64');
