...
export async function enhancePromptForMediaGeneration(
  userPrompt: string,
  ...
  if (customSystemPrompt) {
    systemPrompt =
      customSystemPrompt +
      '\n\nTransformiere den folgenden Benutzertext in einen detaillierten, professionellen Prompt für die ' +
      (mediaType === 'image' ? 'Bildgenerierung' : 'Videogenerierung') +
  ...
  try {
    ...
    console.log(`Prompt enhanced for ${mediaType}:`, {
      original: userPrompt,
...
export async function generateWithOpenAI(
  ...
  if (type === 'image') {
    try {
      ...
      throw new Error(
        `Fehler bei der Bildgenerierung mit DALL-E 3: ${errorMessage}`
      );
    }
  }
