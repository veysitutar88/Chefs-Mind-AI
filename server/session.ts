...
export async function buildSession(env: Env): Promise<RequestHandler> {
  ...
  if (env === 'production') {
    ...
    if (allowPgInProd) {
      ...
      log('[session][postgres] ALLOW_NONREDIS_IN_PROD=true: Using PostgresSessionStore for production fallback', 'warn', 'session');
      ...
      log(`[session][postgres] Using store=${(store as any).constructor.name}, secure=${sessionConfig.cookie.secure}, domain=${sessionConfig.cookie.domain}, env=production`, 'info', 'session');

      const cookieOpts = sessionConfig.cookie as session.CookieOptions;
      const middleware = session(sessionConfig);
      log(`[sessions] store=${(store as any).constructor.name}, ${cookieInfoForLog(cookieOpts)}, env=${env}`, 'info', 'session');
      return middleware;
    }
    ...
    redisClient.on('error', (err) => log(`[session][redis] Client error: ${err?.message ?? ''}`, 'error', 'session'));
    ...
    if (store.constructor.name !== 'RedisStore') {
      log('[sessions] FATAL: MemoryStore or non-Redis store detected in production', 'error', 'session');
      process.exit(1);
    }
    ...
    const cookieOpts = sessionConfig.cookie as session.CookieOptions;
    const middleware = session(sessionConfig);
    log(`[sessions] store=${store.constructor.name}, ${cookieInfoForLog(cookieOpts)}, env=${env}`, 'info', 'session');
    return middleware;
  } else {
