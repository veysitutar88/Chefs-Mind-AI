  ...
  async (req: QARequest, res: Response) => {
    try {
      ...
      try {
        ...
        // Метрика успешной генерации
        const endTime = Date.now();
        const duration = endTime - startTime;
        httpLatency
          .labels('POST', '/api/media/image/generate', '200')
          .observe(duration);
      } catch (error) {
        // Метрика ошибки генерации
        const endTime = Date.now();
        const duration = endTime - startTime;
        httpLatency
          .labels('POST', '/api/media/image/generate', '500')
          .observe(duration);
        mediaGenerationErrors
          .labels('image', generator || defaultProvider, 'error')
          .inc();

        // Если включен fallback и основной генератор не сработал
        if (allowFallback && generator && generator !== defaultProvider) {
          console.log(
            `🔄 Fallback: trying ${defaultProvider} after ${generator} failed`
          );
          mediaGenerationFallbacks
            .labels('image', generator, defaultProvider)
            .inc();
