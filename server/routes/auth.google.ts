...
import {
  ...
  listSpreadsheets,
  createEvent,
} from '../services/google-mcp.js';

const r = Router();

// session middleware теперь подключается глобально в auth.ts

// старт авторизации (корневой маршрут для smoke тестов)
r.get('/', (_req, res) => {
...
r.get('/callback', async (req, res) => {
  try {
    ...
    res.send(
      `<html><body>Google OAuth: OK. Можно закрыть вкладку. <a href="/auth/google/status" target="_blank">Status</a></body></html>`
    );
  } catch (e: any) {
...
r.post('/test-event', ensureAuthed, async (req, res) => {
  try {
    const {
      summary = 'Test Event',
      startTime,
      description = 'Test event created via smoke test',
    } = req.body;
...
r.get('/calendars', ensureAuthed, async (_req, res) => {
  try {
    res.json({ ok: true, items: await listCalendars() });
  } catch (e: any) {
...
r.post('/docs', ensureAuthed, async (req, res) => {
  try {
    res.json({
      ok: true,
      doc: await createDoc(req.body?.title || "Chef's Mind — Doc"),
    });
  } catch (e: any) {
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
...
r.post('/sheets', ensureAuthed, async (req, res) => {
  try {
    res.json({
      ok: true,
      sheet: await createSheet(req.body?.title || "Chef's Mind — Sheet"),
    });
  } catch (e: any) {
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
...
r.get('/spreadsheets', ensureAuthed, async (_req, res) => {
  try {
    res.json({ ok: true, items: await listSpreadsheets() });
  } catch (e: any) {
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
