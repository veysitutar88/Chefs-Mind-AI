...
router.post('/api/universal-ask', requireRole(['chef']), async (req, res) => {
  try {
    const { role, query, systemPrompt, timeoutMs } = req.body || {};
    if (!role || !query) {
      return res
        .status(400)
        .json({ success: false, error: 'role and query are required' });
    }
...
router.post('/api/universal-ask-test', async (req, res) => {
  try {
    const { role, query, systemPrompt, timeoutMs } = req.body || {};
    if (!role || !query) {
      return res
    ...
    console.log(
      `[universal-ask-test] Testing with role: ${role}, query: ${query}`
    );
    const result = await universalAsk({ role, query, systemPrompt });
