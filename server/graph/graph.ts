...
function classifyByKeywords(text: string): {
  ...
  const sorted = (Object.entries(scores) as [AgentKey, number][]).sort(
    (a, b) => b[1] - a[1]
  );
...
function normalizeAgentName(name: string): AgentKey | null {
  ...
  if (n.includes('chef') || n.includes('повар') || n.includes('шеф'))
    return 'chef';
  if (n.includes('research') || n.includes('аналит') || n.includes('исслед'))
    return 'researcher';
  switch (n) {
    case 'media_studio':
    case 'media-studio':
...
export async function orchestratorNode(
  ...
  const confident =
    ...
    (heuristic.bestScore >= 2 ||
      (heuristic.bestScore >= 1 && heuristic.secondBest === 0));
...
// Узел Шефа
export async function chefNode(
  state: GraphStateType
): Promise<Partial<GraphStateType>> {
  const lastMessage = state.messages[state.messages.length - 1];
  ...
  try {
    ...
    return {
      agentOutcome: 'error',
      metadata: {
        error: error instanceof Error ? error.message : 'Chef node error',
      },
    };
  }
}
...
// Узел Учётчика
export async function accountantNode(
  state: GraphStateType
): Promise<Partial<GraphStateType>> {
  const lastMessage = state.messages[state.messages.length - 1];
  ...
  try {
    ...
    return {
      agentOutcome: 'error',
      metadata: {
        error: error instanceof Error ? error.message : 'Accountant node error',
      },
    };
  }
}
...
export async function researcherNode(
  ...
  try {
    ...
    return {
      agentOutcome: 'error',
      metadata: {
        error: error instanceof Error ? error.message : 'Researcher node error',
      },
    };
  }
}
...
// Узел Медиа-студии
export async function mediaStudioNode(
  state: GraphStateType
): Promise<Partial<GraphStateType>> {
  const lastMessage = state.messages[state.messages.length - 1];
  ...
  try {
    ...
    return {
      ...
      metadata: {
        error:
          error instanceof Error ? error.message : 'Media Studio node error',
      },
    };
  }
}
...
export async function processWithAgents(message: string): Promise<{
  ...
  try {
    ...
    if (
      orchestratorResult.agentOutcome === 'error' ||
      !orchestratorResult.currentAgent
    ) {
      throw new Error(
        orchestratorResult.metadata?.error || 'Failed to determine agent'
      );
    }
