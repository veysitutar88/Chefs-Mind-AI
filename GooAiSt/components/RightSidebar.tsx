...
import { FileItem, TodoItem } from '../types';
    ...
    <div className="h-full flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 max-h-[35%]">
        <div className="flex items-center justify-between px-2">
          ...
          <button className="p-1.5 rounded-lg hover:bg-surface text-textSecondary hover:text-accent transition-colors" title="Upload File">
            <Upload size={14} />
          </button>
        </div>
        
        <div className="flex-1 bg-surface/50 rounded-[22px] border border-borderSoft p-4 overflow-y-auto space-y-3 min-h-0">
          {files.map(file => {
             let Icon = File;
             ...
             return (
              <div key={file.id} className="group flex items-start gap-3 p-2 rounded-xl hover:bg-surface border border-transparent hover:border-borderSoft transition-all cursor-pointer">
                <div className="p-1.5 bg-background rounded-lg text-accentSoft">
                  <Icon size={14} />
                </div>
      ...
      <div className="flex-1 flex flex-col gap-4 min-h-0">
        <div className="flex items-center justify-between px-2">
          ...
          <span className="text-[10px] bg-surface border border-borderSoft px-2 py-0.5 rounded-full text-textSecondary">
            {todos.filter(t => t.completed).length}/{todos.length}
          </span>
        </div>

        <div className="flex-1 bg-surface/50 rounded-[22px] border border-borderSoft p-4 flex flex-col min-h-0">
          ...
          <form onSubmit={handleAddSubmit} className="flex gap-2 mb-4">
            <input
              ...
              className="flex-1 bg-background border border-borderSoft rounded-lg px-3 py-2 text-xs text-textPrimary focus:outline-none focus:border-accent placeholder:text-textSecondary/50"
            />
            <button 
              type="submit"
              disabled={!newTodoText.trim()}
              className="bg-accent text-white p-2 rounded-lg hover:bg-accentSoft disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              <Plus size={14} />
            </button>
          </form>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${todo.completed 
                      ...
                      : 'bg-transparent border-borderSoft hover:border-accent text-transparent'
                    }
                  `}
                >
      ...
      <div className="flex flex-col gap-4 shrink-0">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button className="flex flex-col items-center justify-center gap-2 p-3 bg-surface rounded-[20px] border border-borderSoft hover:border-accent/50 hover:bg-[#1A2438] transition-all group">
